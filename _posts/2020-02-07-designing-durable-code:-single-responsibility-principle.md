...
<p>&emsp;This strategy involves rephrasing each of a class's methods as a question and posing each question to the class. For example, in the case of my Minesweeper program the Tile class contains a method that flags a tile. I may therefore ask the Tile class, <em>"Mr. Tile, are you flagged?"</em> You can tell if a method is related to a class's central purpose if its corresponding question makes sense. If I were to ask the same question of the Board class, (<em>"Mr. Board, are you flagged?"</em>) it's clear that .
